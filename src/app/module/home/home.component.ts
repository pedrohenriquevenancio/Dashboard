...
export class HomeComponent implements OnInit {
  ...
  constructor(private api: ApiService, private characterService: CharactersService) { }

  ngOnInit() {
    this.api.getAllData('characters').subscribe(characters => {
      this.characters = characters;
      this.characterService.setCharacter(characters[0]);
      this.sortedChildrenLengths = characters.map(character => character.children.length).sort((a, b) => b - a);
